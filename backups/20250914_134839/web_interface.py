...
<body>
        ...
        <div class="dashboard">
            ...
            <div class="card query-section">
                ...
                <div style="display: flex; gap: 10px; margin-bottom: 20px;">
                    ...
                    <button class="btn" onclick="clearHistory()" style="background: #ff4757;">
                        🗑️ Clear History
                    </button>
                </div>
    ...
    <script>
        ...
        async function processQuery() {
            ...
            // Vérifier si c'est la première requête ou s'il y a déjà du contenu
            const currentContent = responseContent.innerHTML;
            if (currentContent.trim() === '' || currentContent.includes('Processing your query...')) {
                responseContent.innerHTML = '<div class="loading"><div class="spinner"></div>Processing your query...</div>';
            } else {
                // Ajouter le spinner à la fin pour les requêtes suivantes
                responseContent.innerHTML += '<div class="loading"><div class="spinner"></div>Processing your query...</div>';
            }
        ...
        function displayQueryResult(result) {
            ...
            // Ajouter la nouvelle réponse à l'historique au lieu de remplacer
            const currentContent = responseContent.innerHTML;
            console.log('Current content length:', currentContent.length);
            console.log('Contains processing:', currentContent.includes('Processing your query...'));
            console.log('Contains response-answer:', currentContent.includes('response-answer'));
            console.log('Contains Answer:', currentContent.includes('Answer:'));
            
            // Si c'est vide ou contient SEULEMENT le spinner (pas de réponse), on remplace
            if (currentContent.trim() === '' || 
                (currentContent.includes('Processing your query...') && !currentContent.includes('Answer:'))) {
                console.log('Replacing content');
                responseContent.innerHTML = html;
            } else {
                console.log('Appending content');
                // Remplacer le dernier spinner par la nouvelle réponse
                const contentWithoutLastSpinner = currentContent.replace(/<div class="loading"><div class="spinner"><\/div>Processing your query\.\.\.<\/div>$/, '');
                responseContent.innerHTML = contentWithoutLastSpinner + '<hr style="margin: 30px 0; border: 1px solid #e1e5e9;">' + html;
            }
        }
